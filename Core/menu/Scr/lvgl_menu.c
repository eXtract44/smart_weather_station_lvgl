...
static void create_block_top_left() {
	...
	lv_obj_set_style_bg_img_src(lv_object[SYMBOL_TEMPERATURE_INSIDE], LV_SYMBOL_NEW_LINE, 0);
	lv_obj_set_style_bg_img_src(lv_object[SYMBOL_HUMIDITY_INSIDE], LV_SYMBOL_TINT, 0);
...
static void create_block_bot_left() {
	...
	lv_obj_set_style_bg_img_src(lv_object[SYMBOL_TEMPERATURE_OUTSIDE], LV_SYMBOL_NEW_LINE, 0);
	lv_obj_set_style_bg_img_src(lv_object[SYMBOL_HUMIDITY_OUTSIDE], LV_SYMBOL_TINT, 0);
...
void draw_weather() {
	...
	if (read_weather_clouds() != status_cloud_old) { /*CLOUDS SUN/MOON*/
		...
		} else if (read_weather_clouds() > 60) {
			lv_obj_add_flag(lv_object[IMAGE_SUN], LV_OBJ_FLAG_HIDDEN);
			lv_obj_add_flag(lv_object[IMAGE_MOON], LV_OBJ_FLAG_HIDDEN);
			lv_obj_clear_flag(lv_object[IMAGE_CLOUD], LV_OBJ_FLAG_HIDDEN);
			lv_obj_clear_flag(lv_object[IMAGE_CLOUD_1], LV_OBJ_FLAG_HIDDEN);
		}
		status_cloud_old = read_weather_clouds();
	}

	if (read_weather_wind() != status_wind_old) { /*WIND*/
		if (read_weather_wind() < 5) {
			lv_obj_add_flag(lv_object[IMAGE_WIND], LV_OBJ_FLAG_HIDDEN);
		} else if (read_weather_wind() > 5) {
			lv_obj_clear_flag(lv_object[IMAGE_WIND], LV_OBJ_FLAG_HIDDEN);
		}
		status_wind_old = read_weather_wind();
	}
	if (read_weather_rain() != status_rain_old) { /*RAIN*/
		if (read_weather_rain() > 1 && read_weather_rain() < 30) {
			lv_obj_clear_flag(lv_object[IMAGE_RAIN], LV_OBJ_FLAG_HIDDEN);
			lv_obj_add_flag(lv_object[IMAGE_RAIN_1], LV_OBJ_FLAG_HIDDEN);
		} else if (read_weather_rain() > 30) {
			lv_obj_clear_flag(lv_object[IMAGE_RAIN], LV_OBJ_FLAG_HIDDEN);
			lv_obj_clear_flag(lv_object[IMAGE_RAIN_1], LV_OBJ_FLAG_HIDDEN);
		}
		status_rain_old = read_weather_rain();
	}
	if (read_weather_snow() != status_snow_old) { /*SNOW*/
		if (read_weather_snow() > 1 && read_weather_snow() < 30) {
			lv_obj_clear_flag(lv_object[IMAGE_SNOW], LV_OBJ_FLAG_HIDDEN);
			lv_obj_add_flag(lv_object[IMAGE_SNOW_1], LV_OBJ_FLAG_HIDDEN);
		} else if (read_weather_snow() > 30) {
			lv_obj_clear_flag(lv_object[IMAGE_SNOW], LV_OBJ_FLAG_HIDDEN);
			lv_obj_clear_flag(lv_object[IMAGE_SNOW_1], LV_OBJ_FLAG_HIDDEN);
		}
		status_snow_old = read_weather_snow();
	}
}
void draw_symbol_battery() {
	...
	if (read_battery_status() != status_battery_old) {
		...
		case 1:
			...
			break;
		case 2:
			lv_obj_set_style_bg_img_src(lv_object[SYMBOL_BATTERY], LV_SYMBOL_BATTERY_1, 0);
			break;
		case 3:
			lv_obj_set_style_bg_img_src(lv_object[SYMBOL_BATTERY], LV_SYMBOL_BATTERY_2, 0);
			break;
		case 4:
			lv_obj_set_style_bg_img_src(lv_object[SYMBOL_BATTERY], LV_SYMBOL_BATTERY_3, 0);
			break;
		case 5:
...
void draw_symbol_wifi() {
	...
	if (read_wifi() != status_wifi_old) {
		...
		case WIFI_CONNECTING:
			lv_obj_set_style_bg_img_src(lv_object[SYMBOL_WIFI], LV_SYMBOL_REFRESH, 0);
			break;
		case WIFI_CONNECTED:
...
void draw_symbol_volume() {
	...
	if (read_volume_dfplayer() != read_volume_old) {
		...
		case 1:
			...
			break;
		case 2:
			lv_obj_set_style_bg_img_src(lv_object[SYMBOL_BUZZER], LV_SYMBOL_VOLUME_MID, 0);
			break;
		case 3:
