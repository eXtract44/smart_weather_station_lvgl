...
static lv_obj_t* create_meter(lv_obj_t *screen, lv_coord_t w, lv_coord_t h,
	...
	indic_co2 = lv_meter_add_arc(meter, scale, 3,
	...
	indic_co2 = lv_meter_add_scale_lines(meter, scale,
	...
	indic_co2 = lv_meter_add_needle_line(meter, scale, 4,
...
static lv_obj_t* create_anim_image_snow_zoom(lv_obj_t *bg,
		const uint8_t zoom_size, lv_align_t align, lv_coord_t x_ofs,
		lv_coord_t y_ofs) {
...
static lv_obj_t* create_anim_image_rain_zoom(lv_obj_t *bg,
		const uint8_t zoom_size, lv_align_t align, lv_coord_t x_ofs,
		lv_coord_t y_ofs) {
...
static lv_obj_t* create_anim_image_cloud_zoom(lv_obj_t *bg,
		const uint8_t zoom_size, lv_align_t align, lv_coord_t x_ofs,
		lv_coord_t y_ofs) {
...
static lv_obj_t* create_anim_image_wind_zoom(lv_obj_t *bg,
		const uint8_t zoom_size, lv_align_t align, lv_coord_t x_ofs,
		lv_coord_t y_ofs) {
...
static lv_obj_t* create_anim_image_sun(lv_obj_t *bg, lv_align_t align,
...
static lv_obj_t* create_image_moon(lv_obj_t *bg, lv_align_t align,
		lv_coord_t x_ofs, lv_coord_t y_ofs) {
...
static void create_block_top_left() {
	...
	/*BLOCK TOP LEFT*/
	para_t bg_top_left = { .width = 150, .height = 130, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = x_start_left_block - 20, .y_ofs = 30 };

	create_text("inside", lv_object[SCREEN_MAIN_MENU], STYLE_TEXT_SMALL,
			bg_top_left.align, bg_top_left.x_ofs + 45, bg_top_left.y_ofs - 20);

	lv_object[BACKGROUND_INSIDE_VALUES] = create_background(
			lv_object[SCREEN_MAIN_MENU], bg_top_left.width, bg_top_left.height,
			bg_top_left.align, bg_top_left.x_ofs, bg_top_left.y_ofs);
	lv_obj_add_style(lv_object[BACKGROUND_INSIDE_VALUES], &style_bg_top_left,
			0);

	para_t symbol_inside = { .width = 20, .height = 20, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = x_start_left_block - 15, .y_ofs = 20 };
	lv_object[SYMBOL_TEMPERATURE_INSIDE] = create_button_symbol(
			lv_object[SCREEN_MAIN_MENU], symbol_inside.width,
			symbol_inside.height, symbol_inside.align, symbol_inside.x_ofs,
			symbol_inside.y_ofs + symbol_inside.height);
	lv_object[SYMBOL_HUMIDITY_INSIDE] = create_button_symbol(
			lv_object[SCREEN_MAIN_MENU], symbol_inside.width,
			symbol_inside.height, symbol_inside.align, symbol_inside.x_ofs,
			symbol_inside.y_ofs + symbol_inside.height * 2 + 10);
	lv_object[SYMBOL_TVOC] = create_button_symbol(lv_object[SCREEN_MAIN_MENU],
			symbol_inside.width, symbol_inside.height, symbol_inside.align,
			symbol_inside.x_ofs,
			symbol_inside.y_ofs + symbol_inside.height * 3 + 20);
	lv_object[SYMBOL_BRIGHTNESS] = create_button_symbol(
			lv_object[SCREEN_MAIN_MENU], symbol_inside.width,
			symbol_inside.height, symbol_inside.align, symbol_inside.x_ofs,
	...
	lv_obj_set_style_bg_img_src(lv_object[SYMBOL_BRIGHTNESS], LV_SYMBOL_TRASH,
			0);

	const int16_t y_offset_inside = 29;
	para_t values_inside = { .align = LV_ALIGN_TOP_LEFT, .x_ofs =
			x_start_left_block + 15, .y_ofs = 40 };
	lv_object[VALUE_TEMPERATURE_INSIDE] = create_label(
			lv_object[SCREEN_MAIN_MENU], "0", values_inside.align,
			values_inside.x_ofs, values_inside.y_ofs);
	lv_obj_add_style(lv_object[VALUE_TEMPERATURE_INSIDE],
			&style[STYLE_TEXT_LARGE], 0);
	create_text("c*", lv_object[SCREEN_MAIN_MENU], STYLE_TEXT_SMALL,
			values_inside.align, values_inside.x_ofs + 90, values_inside.y_ofs);
	lv_object[VALUE_HUMIDITY_INSIDE] = create_label(lv_object[SCREEN_MAIN_MENU],
			"0 %*", values_inside.align, values_inside.x_ofs,
			values_inside.y_ofs + y_offset_inside + 4);
	lv_object[VALUE_TVOC_INSIDE] = create_label(lv_object[SCREEN_MAIN_MENU],
			"0 ppm", values_inside.align, values_inside.x_ofs,
			values_inside.y_ofs + y_offset_inside * 2 + 4);
	lv_object[VALUE_BRIGHTNESS_INSIDE] = create_label(
			lv_object[SCREEN_MAIN_MENU], "0 lux", values_inside.align,
...
static void create_block_top_middle() {
	/*BLOCK TOP MID*/
	para_t meter_co2 = { .width = 150, .height = 150, .align = LV_ALIGN_TOP_MID,
			.x_ofs = 0, .y_ofs = 15 };
	para_t co2_value = { .align = LV_ALIGN_TOP_MID, .x_ofs = 0, .y_ofs = 120 };
	lv_object[METER_CO2_MAIN_MENU] = create_meter(lv_object[SCREEN_MAIN_MENU],
			meter_co2.width, meter_co2.height, meter_co2.align, meter_co2.x_ofs,
			meter_co2.y_ofs);
	...
	create_text("ppm", lv_object[SCREEN_MAIN_MENU], STYLE_TEXT_SMALL,
			meter_co2.align, meter_co2.x_ofs, meter_co2.y_ofs + 45);

	lv_object[VALUE_CO2_INSIDE] = create_label(lv_object[SCREEN_MAIN_MENU], "0",
			co2_value.align, co2_value.x_ofs, co2_value.y_ofs);
...
static void create_block_top_right() {
	...
	para_t bg_top_right = { .width = 150, .height = 130, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = 320, .y_ofs = 30 };
	para_t time_ = { .align = LV_ALIGN_TOP_LEFT, .x_ofs = 316, .y_ofs = 36 };

	lv_object[BACKGROUND_CLOCK_VALUES] = create_background(
			lv_object[SCREEN_MAIN_MENU], bg_top_right.width,
			bg_top_right.height, bg_top_right.align, bg_top_right.x_ofs,
			bg_top_right.y_ofs);
	lv_obj_add_style(lv_object[BACKGROUND_CLOCK_VALUES], &style_bg_top_right,
			0);

	create_text("time", lv_object[SCREEN_MAIN_MENU], STYLE_TEXT_SMALL,
			bg_top_right.align, bg_top_right.x_ofs + 55,
			bg_top_right.y_ofs - 20);

	lv_object[TIME_HOUR_MINUTE] = create_label(lv_object[SCREEN_MAIN_MENU],
			"00:00", time_.align, time_.x_ofs, time_.y_ofs);
	lv_obj_add_style(lv_object[TIME_HOUR_MINUTE], &style[STYLE_TEXT_VERY_LARGE],
			0);

	lv_object[TIME_MDAY_MONTH] = create_label(lv_object[SCREEN_MAIN_MENU],
			"00.00", time_.align, time_.x_ofs, time_.y_ofs + time_offset_y);
	lv_obj_add_style(lv_object[TIME_MDAY_MONTH], &style[STYLE_TEXT_VERY_LARGE],
			0);

	lv_object[TIME_WDAY] = create_label(lv_object[SCREEN_MAIN_MENU], "load...",
			time_.align, time_.x_ofs + 70, time_.y_ofs + time_offset_y * 2 - 5);
	/*BLOCK TOP RIGHT*/
}
static void create_block_bot_left() {
	...
	para_t bg_outside = { .width = 150, .height = 130, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = x_start_left_block - 20, .y_ofs = 180 };
	create_text("outside", lv_object[SCREEN_MAIN_MENU], STYLE_TEXT_SMALL,
			bg_outside.align, bg_outside.x_ofs + 45, bg_outside.y_ofs - 20);

	lv_object[BACKGROUND_OUTSIDE_VALUES] = create_background(
			lv_object[SCREEN_MAIN_MENU], bg_outside.width, bg_outside.height,
			bg_outside.align, bg_outside.x_ofs, bg_outside.y_ofs);
	lv_obj_add_style(lv_object[BACKGROUND_OUTSIDE_VALUES], &style_bg_bot_left,
			0);

	para_t symbol_outside = { .width = 20, .height = 20, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = x_start_left_block - 15, .y_ofs = 170 };
	lv_object[SYMBOL_TEMPERATURE_OUTSIDE] = create_button_symbol(
			lv_object[SCREEN_MAIN_MENU], symbol_outside.width,
	...
	lv_object[SYMBOL_WIND] = create_button_symbol(lv_object[SCREEN_MAIN_MENU],
			symbol_outside.width, symbol_outside.height, symbol_outside.align,
			symbol_outside.x_ofs,
			symbol_outside.y_ofs + symbol_outside.height * 3 + 20);
	lv_object[SYMBOL_PRESS] = create_button_symbol(lv_object[SCREEN_MAIN_MENU],
			symbol_outside.width, symbol_outside.height, symbol_outside.align,
			symbol_outside.x_ofs,
	...
	lv_obj_set_style_bg_img_src(lv_object[SYMBOL_PRESS], LV_SYMBOL_BACKSPACE,
			0);

	para_t values_outside = { .align = LV_ALIGN_TOP_LEFT, .x_ofs =
			x_start_left_block + 15, .y_ofs = 190 };
	lv_object[VALUE_TEMPERATURE_OUTSIDE] = create_label(
			lv_object[SCREEN_MAIN_MENU], "0 c*", values_outside.align,
			values_outside.x_ofs, values_outside.y_ofs);
	lv_obj_add_style(lv_object[VALUE_TEMPERATURE_OUTSIDE],
			&style[STYLE_TEXT_LARGE], 0);
	create_text("c*", lv_object[SCREEN_MAIN_MENU], STYLE_TEXT_SMALL,
			values_outside.align, values_outside.x_ofs + 90,
			values_outside.y_ofs);
	lv_object[VALUE_HUMIDITY_OUTSIDE] = create_label(
			lv_object[SCREEN_MAIN_MENU], "0 %", values_outside.align,
			values_outside.x_ofs, values_outside.y_ofs + y_offset_outside + 4);
	lv_object[VALUE_WIND_OUTSIDE] = create_label(lv_object[SCREEN_MAIN_MENU],
			"0 m/s", values_outside.align, values_outside.x_ofs,
			values_outside.y_ofs + y_offset_outside * 2 + 4);
	lv_object[VALUE_PRESS_OUTSIDE] = create_label(lv_object[SCREEN_MAIN_MENU],
			"0 hPa", values_outside.align, values_outside.x_ofs,
			values_outside.y_ofs + y_offset_outside * 3 + 4);
	/*BLOCK BOT LEFT*/
}
static void create_block_bot_middle() {
	...
	/*BLOCK BOT MID*/
	para_t chart_co2 = { .width = 150, .height = 70, .align = LV_ALIGN_TOP_MID,
			.x_ofs = 0, .y_ofs = 180 };

	create_text("co2           24h", lv_object[SCREEN_MAIN_MENU],
			STYLE_TEXT_SMALL, chart_co2.align, chart_co2.x_ofs,
			chart_co2.y_ofs - 20);

	lv_object[CHART_CO2_MAIN_MENU] = create_chart(lv_object[SCREEN_MAIN_MENU],
			chart_co2.width, chart_co2.height, chart_co2.align, chart_co2.x_ofs,
			chart_co2.y_ofs);
	lv_obj_add_style(lv_object[CHART_CO2_MAIN_MENU], &style_chart_co2, 0);

	para_t symbol_notification = { .width = 30, .height = 30, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = 170, .y_ofs = 275 };
	lv_object[SYMBOL_EYE] = create_button_symbol(lv_object[SCREEN_MAIN_MENU],
			symbol_notification.width, symbol_notification.height,
			symbol_notification.align, symbol_notification.x_ofs,
			symbol_notification.y_ofs);
	lv_object[SYMBOL_BUZZER] = create_button_symbol(lv_object[SCREEN_MAIN_MENU],
			symbol_notification.width, symbol_notification.height,
			symbol_notification.align,
			symbol_notification.x_ofs + symbol_notification.width + 4,
			symbol_notification.y_ofs);
	lv_object[SYMBOL_WIFI] = create_button_symbol(lv_object[SCREEN_MAIN_MENU],
			symbol_notification.width, symbol_notification.height,
			symbol_notification.align,
			symbol_notification.x_ofs + symbol_notification.width * 2 + 8,
			symbol_notification.y_ofs);
	lv_object[SYMBOL_BATTERY] = create_button_symbol(
			lv_object[SCREEN_MAIN_MENU], symbol_notification.width,
			symbol_notification.height, symbol_notification.align,
			symbol_notification.x_ofs + symbol_notification.width * 3 + 16,
			symbol_notification.y_ofs);
	para_t bar_distance = { .width = 150, .height = 10, .align =
			LV_ALIGN_TOP_LEFT, .x_ofs = 170, .y_ofs = 310 };
	...
	lv_obj_align(lv_object[BAR_DISTANCE], bar_distance.align,
			bar_distance.x_ofs, bar_distance.y_ofs);
	lv_obj_set_size(lv_object[BAR_DISTANCE], bar_distance.width,
			bar_distance.height);
	lv_bar_set_range(lv_object[BAR_DISTANCE], 1000, 0);
	/*BLOCK BOT MID*/
}
static void create_block_bot_right() {
	/*BLOCK BOT RIGHT*/
	para_t symbol_weather = { .align = LV_ALIGN_TOP_LEFT, .x_ofs = 335, .y_ofs =
			170 };

	lv_object[IMAGE_SUN] = create_anim_image_sun(lv_object[SCREEN_MAIN_MENU],
			symbol_weather.align, symbol_weather.x_ofs + 70,
			symbol_weather.y_ofs + 5);
	lv_object[IMAGE_MOON] = create_image_moon(lv_object[SCREEN_MAIN_MENU],
			symbol_weather.align, symbol_weather.x_ofs + 70,
			symbol_weather.y_ofs + 5);
	lv_object[IMAGE_CLOUD] = create_anim_image_cloud_zoom(
			lv_object[SCREEN_MAIN_MENU], 30, symbol_weather.align,
			symbol_weather.x_ofs, symbol_weather.y_ofs + 10);
	lv_object[IMAGE_CLOUD_1] = create_anim_image_cloud_zoom(
			lv_object[SCREEN_MAIN_MENU], 50, symbol_weather.align,
			symbol_weather.x_ofs + 20, symbol_weather.y_ofs + 60);
	lv_object[IMAGE_WIND] = create_anim_image_wind_zoom(
			lv_object[SCREEN_MAIN_MENU], 30, symbol_weather.align,
			symbol_weather.x_ofs + 10, symbol_weather.y_ofs + 80);
	lv_object[IMAGE_RAIN] = create_anim_image_rain_zoom(
			lv_object[SCREEN_MAIN_MENU], 30, symbol_weather.align,
			symbol_weather.x_ofs, symbol_weather.y_ofs + 40);
	lv_object[IMAGE_RAIN_1] = create_anim_image_rain_zoom(
			lv_object[SCREEN_MAIN_MENU], 50, symbol_weather.align,
			symbol_weather.x_ofs + 60, symbol_weather.y_ofs + 80);
	lv_object[IMAGE_SNOW] = create_anim_image_snow_zoom(
			lv_object[SCREEN_MAIN_MENU], 10, symbol_weather.align,
			symbol_weather.x_ofs + 40, symbol_weather.y_ofs + 40);
	lv_object[IMAGE_SNOW_1] = create_anim_image_snow_zoom(
			lv_object[SCREEN_MAIN_MENU], 50, symbol_weather.align,
			symbol_weather.x_ofs + 60, symbol_weather.y_ofs + 80);
...
static void create_menu() {
	...
	lv_object[BACKGROUND_MAIN_MENU] = lv_obj_create(
			lv_object[SCREEN_MAIN_MENU]);
...
void draw_symbol_wifi() {
	...
	if (read_wifi() != status_wifi_old) {
		...
		case WIFI_CONNECTING:
			...
			break;
		case WIFI_CONNECTED:
			lv_obj_set_style_bg_img_src(lv_object[SYMBOL_WIFI], LV_SYMBOL_WIFI,
					0);
			break;
...
void update_block_top_left() {
	aht10_read();
	sgp30_read();
	print_value("%.1f", read_temperature_aht10(),
			lv_object[VALUE_TEMPERATURE_INSIDE]);
	print_value("%.f %%", read_humidity_aht10(),
...
void update_block_top_middle() {
	...
	lv_meter_set_indicator_value(lv_object[METER_CO2_MAIN_MENU], indic_co2,
			read_co2_sgp30());
}
...
void init_lv_objects() {
	...
	create_menu();
	draw_menu_main();
	lv_timer_create(timer_2000, 2000, NULL);
	lv_timer_create(timer_300, 300, NULL);
